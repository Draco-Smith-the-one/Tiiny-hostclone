...
import { Upload, File, CheckCircle, ExternalLink, Loader2 } from 'lucide-react';
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] flex flex-col items-center justify-center p-6 font-sans text-slate-900">
      ...
      {/* Upload Card */}
      <div className="w-full max-w-lg bg-white rounded-2xl shadow-xl shadow-indigo-100/50 border border-slate-100 p-8">
        <div 
        ...
        >
          ...
          <p className="text-xs text-slate-400 mt-1">HTML, ZIP, JPG or PNG</p>
        </div>

        <button 
          onClick={handleUpload} 
          disabled={uploading || !file}
          className={`w-full mt-6 py-4 rounded-xl font-bold text-white transition-all transform active:scale-[0.98]
            ${uploading || !file 
              ? 'bg-slate-300 cursor-not-allowed' 
              : 'bg-indigo-600 hover:bg-indigo-700 shadow-lg shadow-indigo-200 hover:-translate-y-0.5'}`}
        >
          {uploading ? "Deploying Site..." : "Launch Now"}
        </button>
      </div>
          ...
          <div className="bg-emerald-50 border border-emerald-100 rounded-2xl p-6 flex items-start gap-4">
            ...
            <div className="flex-1 overflow-hidden">
              ...
              <a 
                ...
                target="_blank" 
                rel="noreferrer"
                className="inline-flex items-center gap-2 bg-white px-4 py-2 rounded-lg text-sm font-bold text-slate-900 border border-slate-200 hover:bg-slate-50 transition-colors"
              >
